...
| `claims_matrix` | `pkg/servers/token/token.go:254-265`, `github.com/golang-jwt/jwt` v3.2.2 | The same claim build over fifteen instants, five requestor ids and three token ids, in `America/Los_Angeles`, in `Europe/Paris` and in UTC: the whole payload, the `expires` string with its instant and offset, and the signing input. The one `Europe/Paris` case is the only one that can fix the order of `AddDate`'s two zone lookups, because a zone west of Greenwich puts both candidate instants on the same side of a transition |
...
| `robot_parse` | `vector-cloud/internal/token/identity/identity.go:158`, `.../token.go:96-161`, `github.com/golang-jwt/jwt` `parser.go:96-148` | The robot's own reader over thirty-five crafted tokens: the six required claims and the optional one, the `time.RFC3339` shapes it takes and refuses, the segment count, the alg lookup, that the signature segment is never decoded, and the four shapes where `parser.go`'s two lenient JSON readers part company with a port that reads each segment as one JSON value. `empty_user_id` is `ok` at parse time and still costs the robot its token: `identity/identity.go:141-145` deletes the token file at boot when the token it reads back carries an empty `user_id` |
...
|---|---|---|---|
| `go-probe/expected.txt` | 459 | 7 | 452 |
