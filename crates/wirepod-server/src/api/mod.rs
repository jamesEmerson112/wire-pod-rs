...
pub async fn handle(State(state): State<Arc<AppState>>, req: Request) -> Response {
    ...
    let mut response = match route.as_str() {
        ...
        "get_logs" => logs::get_logs(&state),
        "get_debug_logs" => logs::get_debug_logs(&state),
        "get_logs_json" => logs::get_logs_json(&state, req.uri().query()),
        ...
        "generate_certs" => certs::generate_certs(&state).await,
