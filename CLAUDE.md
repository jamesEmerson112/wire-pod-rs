...
| `wirepod-vector` | `vector-go-sdk` usage | Outbound robot client: ExternalInterface RPCs, `update_settings` REST, port 8889 consolevar. The P4 slice landed the tonic client, the stream adapters, the error conversion, the connection factory and the loopback fake; the REST and consolevar calls are still to come. |
